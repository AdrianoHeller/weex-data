...
app.post("/apiweex/login", async (req, res): Promise<any> => {
  ...
  if (req.method === "POST") {
    ...
    if (user.length > 0) {
      ...
      res
        .status(404)
        .send(JSON.stringify({ Message: "No user registered in database." }));
    }
  } else {
    ...
    if (
      ...
      try {
        ...
        if (data !== null) {
          ...
          await cursor
            .collection(String(req.body["EMPRESA"]).toLowerCase())
            .insertOne(req.body)
            .then((resp) => {
      ...
      if (typeof req.body === "object" && req.body instanceof Array) {
        try {
          req.body.forEach(async (item, index) => {
            const checkUserAlreayExist = await cursor
              .collection("login")
              .find({ EMAIL: req.body[index]["EMAIL"] })
              .toArray();
...
app.delete("/apiweex/usuarios/:empresa/:id", async (req, res) => {
  ...
  if (req.method === "DELETE") {
    try {
      ...
      const loginData = await cursor
        .collection("login")
        .findOneAndDelete({ USER_ID: new ObjectId(id) });
      return res
        .status(204)
        .send(
          JSON.stringify({ company_data: companyData, login_data: loginData })
        );
    } catch (err) {
      return res.send(err);
    }
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, callback) => {
    const filteredPic = filesNameFilter(
      `${req.params.user_id}${req.params.flag}`
    );
    ...
    callback(
      ...
      "WEEX" +
        "-" +
        `${Math.floor(Math.random() * 100)}-${req.params.user_id}${
          req.params.flag
        }.${processedMimetype}`
    );
  },
...
app.get("/apiweex/avatar/:imageName", (req, res) => {
  ...
  res
    .status(400)
    .send({ message: `${req.params.imageName} does not exist` })
    .end();
});
...
app.post("/apiweex/usuarios/recuperar_senha", async (req, res) => {
  ...
  try {
    ...
    await cursor.collection("USER").findOneAndUpdate(user.id, {
      $set: {
